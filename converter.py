...
convert = Label(text="Convert",height=5,width=10)
convert.grid(row=0,column=0)
var1 = StringVar()
OptionMenu(window,var1,*short_name).grid(row=0,column=1)
to_convert = Label(text="Convert to",height=5,width=10).grid(row=1,column=0)
var2 = StringVar()
OptionMenu(window,var2,*short_name).grid(row=1,column=1)
am = Text(window,height=1,width=10)
am.grid(row=2,column=1)
Label(text="Amount").grid(row=2,column=0)
b3 = Button(text="Generate",command=lambda: generate(2)).grid(row=3,column=0)
l3 = Label(text="",width= 20,height=3)
l3.grid(row=5,column=0)
Label(text="Unit Price").grid(row=3,column=1)
